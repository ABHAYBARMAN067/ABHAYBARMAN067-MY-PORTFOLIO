...
interface MyWorkButtonProps {
  onClick?: () => void;
  targetId?: string;
}

const MyWorkButton: React.FC<MyWorkButtonProps> = ({ onClick, targetId = 'projects' }) => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    // Default behavior: scroll to the projects section.
        ...
        >
          <AnimatePresence mode="wait">
            {status === 'idle' && (
              ...
              >
                <span className="font-black tracking-wider">MY </span>
                <div
                ...
                <span className="flex items-center gap-0.5 font-black tracking-wider">
                  WORK <ArrowDownToLine size={15} className="transition-transform group-hover:translate-x-0.5" />
                </span>
              </motion.div>
            )}

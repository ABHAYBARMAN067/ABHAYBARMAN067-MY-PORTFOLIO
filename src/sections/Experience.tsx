...
const timeline = [
...

  {
    ...
    period: '2022 - 2023',
    description: 'Built strong foundation in web development through structured learning and hands-on practice in frontend and backend technologies.',
    ...
    <section id="experience" ref={ref} className={`py-28 px-6 relative transition-colors duration-500 ${isDark ? 'bg-black' : 'bg-light-bg'}`}>
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="relative max-w-3xl">
          <motion.div className={`absolute left-5 top-0 bottom-0 w-px ${isDark ? 'bg-gradient-to-b from-white/20 via-white/10 to-transparent' : 'bg-gradient-to-b from-black/20 via-black/10 to-transparent'}`} initial={{ scaleY: 0 }} animate={visible ? { scaleY: 1 } : {}} transition={{ duration: 1.5, delay: 0.3, ease: 'easeOut' }} style={{ transformOrigin: 'top' }} />
          <motion.div className={`absolute left-[18px] w-2 h-2 rounded-full ${isDark ? 'bg-white/60' : 'bg-black/60'}`} initial={{ top: 0, opacity: 0 }} animate={visible ? { top: '100%', opacity: [0, 1, 1, 0] } : {}} transition={{ duration: 3, delay: 0.5, ease: 'easeInOut' }} />
              ...
              <motion.div key={title} className="relative flex gap-6" initial={{ opacity: 0, x: -20 }} animate={visible ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 + i * 0.15 }}>
                ...
                <motion.div className="glass glass-hover rounded-xl p-5 flex-1 group" whileHover={{ x: 4, scale: 1.01 }} transition={{ type: 'spring', stiffness: 200 }}>
                  <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                    ...
                    <span className={`px-2.5 py-1 rounded-full border text-xs font-mono flex-shrink-0 ${isDark ? 'border-white/10 text-white/30' : 'border-black/10 text-black/30'}`}>{period}</span>
                  </div>
                  <p className={`text-sm leading-relaxed mb-4 ${isDark ? 'text-white/40' : 'text-black/40'}`}>{description}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {tags.map((tag) => (
                      <motion.span key={tag} className={`px-2 py-0.5 rounded border text-xs font-mono ${isDark ? 'border-white/8 bg-white/[0.03] text-white/30' : 'border-black/8 bg-black/[0.03] text-black/30'}`} whileHover={{ scale: 1.08 }}>
                        {tag}
                      </motion.span>
                    ))}
                  </div>
